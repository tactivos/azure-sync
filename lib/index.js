  ...
  return new Promise((resolve, reject) => {
    ...
    if (err) return reject(`Error creating container: ${err}`);
      ...
      service.setServiceProperties(cfg.container.properties, err => {
        ...
        glob(`${cfg.source}/**`, (err, files) => {
          if (err) return reject(`Error reading '${cfg.source}' directory: ${err}`);

          var progressBar = new Progress('[:bar] (:current/:total) :percent | Elapsed :elapseds - ETA :etas', { total: files.length, width: 50 });
          const tickAndContinue = cb => { progressBar.tick(); cb(); };

          async.each(files, (file, nextFile) => {
            if (fs.lstatSync(file).isDirectory()) return tickAndContinue(nextFile);

            const blobName = file.replace(cfg.source + path.sep, '');
          ...
          err => {
            ...
            console.log(chalk.green(`Completed Azure Storage Sync for source: ${cfg.source}`));
